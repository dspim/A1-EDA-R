...
- 利用UI安裝`RSQLite`、`ggplot2`、`magrittr`和`dplyr`套件
...
- 放置檔案至data資料夾之下（可跟隨講師操作）

--- &vcenter .largecontent

## 讀取表格檔案 - 1. 先檢視資料

```{r read.lines, echo = TRUE}
...
```{r read.table, echo = TRUE, results = "none"}
...
## 2014 年影響R 使用者們最大的套件之一：magrittr
...
- 讓我們先透過vignette來簡易學習，之後的範例中再做細部解釋
...
- 個數：`length`
- 平均：`mean`、標準差：`sd`、中位數：`median`
- 可能的值的個數：`length(unique)`
- 可能的值：`paste(unique, collapse = ",")`
...
```{r ubike.site.rainfall, echo = TRUE, cache=TRUE}
